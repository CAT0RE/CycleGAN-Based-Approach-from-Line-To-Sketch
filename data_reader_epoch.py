...
# 随机拉伸
def random_resize(img, thresh=0.5):
    if random.random() > thresh:
        type = [Image.NEAREST,Image.BILINEAR,Image.BICUBIC,Image.ANTIALIAS]
        w, h = img.size[0], img.size[1]
        if random.random() > thresh:
            img = img.resize((h, int(w * np.random.uniform(1, 1.4))), type[random.randint(0,3)])
            img = img.crop([0, 0, w, h])
        else:
            img = img.resize((int(h * np.random.uniform(1, 1.4)), w), type[random.randint(0,3)])
            img = img.crop([0, 0, w, h])
    return img
...
# 随机加高斯噪声
def random_noise(img, max_sigma = 1.):
    img = np.asarray(img)
...
def imgaug(img):
    # ops = [random_rotate]
    ops = [random_brightness, random_contrast, random_color, random_sharpness, random_flip, 
            random_resize, random_rotate]
    # ops = [random_flip, random_resize, random_rotate]
